...
impl Guid {
    ...
    pub const fn from_str(s: &str) -> Option<Self> {
        ...
        let d1 = match parse_hex_u32(&[bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7]]) {
            Some(v) => v,
            None => return None,
        };
        ...
        Some(Self([
            d1_bytes[0], d1_bytes[1], d1_bytes[2], d1_bytes[3],
            d2_bytes[0], d2_bytes[1],
            d3_bytes[0], d3_bytes[1],
            d4_0, d4_1,
...
impl Guid {
    ...
    pub const WINDOWS_LDM_DATA: Self = Self([
        0xa0, 0x60, 0x9b, 0xaf, 0x31, 0x14, 0x62, 0x4f,
    ...
    pub const WINDOWS_STORAGE_SPACES: Self = Self([
        0xe7, 0x5c, 0xaf, 0xe7, 0xa0, 0x1a, 0x4d, 0x4d,
        0xbe, 0xe7, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);
    ...
    pub const LINUX_ROOT_X86: Self = Self([
        0x10, 0xd7, 0xda, 0x44, 0x96, 0xe5, 0xe9, 0x4c,
        0xb1, 0x6b, 0x00, 0xa5, 0x24, 0x00, 0x00, 0x00,
    ]);

    /// Linux Root Partition (x86-64).
    pub const LINUX_ROOT_X86_64: Self = Self([
        0x02, 0xb9, 0x2f, 0x4f, 0xbe, 0x39, 0x3e, 0x4e,
        0xb2, 0xc8, 0x23, 0x54, 0x61, 0x6c, 0x02, 0x72,
    ]);

    /// Linux Root Partition (ARM).
    pub const LINUX_ROOT_ARM: Self = Self([
        0xb1, 0x21, 0xb0, 0x69, 0xda, 0xf3, 0x4c, 0x4c,
        0x8d, 0x5a, 0x52, 0x57, 0x4c, 0x02, 0x37, 0x00,
    ]);

    /// Linux Root Partition (ARM64/AArch64).
    pub const LINUX_ROOT_ARM64: Self = Self([
        0xd5, 0x7e, 0x44, 0xb7, 0x85, 0x00, 0x4c, 0x49,
        0x8a, 0x82, 0x7f, 0x05, 0x39, 0x45, 0x00, 0x00,
    ]);
    ...
    pub const LINUX_HOME: Self = Self([
        0x33, 0xe7, 0xd1, 0x93, 0xd8, 0x0c, 0x4d, 0x4e,
        0x90, 0x4a, 0xac, 0x2d, 0x04, 0x00, 0x00, 0x00,
    ]);

    /// Linux /srv (Server Data) Partition.
    pub const LINUX_SRV: Self = Self([
        0x33, 0xe7, 0xd1, 0x93, 0xd8, 0x0c, 0x4d, 0x4e,
        0x90, 0x4a, 0xac, 0x2d, 0x05, 0x00, 0x00, 0x00,
    ]);

    /// Linux dm-crypt / LUKS Partition.
    pub const LINUX_LUKS: Self = Self([
        0x7f, 0xff, 0xff, 0xca, 0xbc, 0xcd, 0x43, 0x4d,
        0xa9, 0x17, 0x87, 0xe1, 0x14, 0x00, 0x00, 0x00,
    ]);
    ...
    pub const FREEBSD_BOOT: Self = Self([
        0x00, 0x08, 0x00, 0x83, 0x01, 0x00, 0xb0, 0x11,
        0x00, 0x00, 0xe4, 0x00, 0x00, 0x69, 0x00, 0x00,
    ]);
    ...
    pub const SOLARIS_BOOT: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x52, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);

    /// Solaris Root Partition.
    pub const SOLARIS_ROOT: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x53, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);

    /// Solaris Swap Partition.
    pub const SOLARIS_SWAP: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x54, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);

    /// Solaris Backup Partition.
    pub const SOLARIS_BACKUP: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);

    /// Solaris /var Partition.
    pub const SOLARIS_VAR: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x56, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);

    /// Solaris /home Partition.
    pub const SOLARIS_HOME: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x57, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);

    /// Solaris Reserved.
    pub const SOLARIS_RESERVED: Self = Self([
        0x45, 0xcb, 0x82, 0x6a, 0xd2, 0x1d, 0x11, 0xd3,
        0x81, 0x5f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);
    ...
    pub const NETBSD_SWAP: Self = Self([
        0x32, 0x8d, 0xf4, 0x49, 0x19, 0xf8, 0xd2, 0x11,
        0xba, 0x4b, 0x00, 0xa0, 0xc9, 0x3e, 0xc9, 0x3b,
    ]);
    ...
    pub const NETBSD_FFS: Self = Self([
        0x32, 0x8d, 0xf4, 0x49, 0x19, 0xf8, 0xd2, 0x11,
        0xba, 0x4c, 0x00, 0xa0, 0xc9, 0x3e, 0xc9, 0x3b,
    ]);

    /// NetBSD LFS Partition.
    pub const NETBSD_LFS: Self = Self([
        0x32, 0x8d, 0xf4, 0x49, 0x19, 0xf8, 0xd2, 0x11,
        0xba, 0x4d, 0x00, 0xa0, 0xc9, 0x3e, 0xc9, 0x3b,
    ]);

    /// NetBSD RAID Partition.
    pub const NETBSD_RAID: Self = Self([
        0x32, 0x8d, 0xf4, 0x49, 0x19, 0xf8, 0xd2, 0x11,
        0xba, 0x4f, 0x00, 0xa0, 0xc9, 0x3e, 0xc9, 0x3b,
    ]);
    ...
    pub const VMWARE_VMFS: Self = Self([
        0x10, 0x04, 0x1d, 0xaa, 0xd1, 0xf4, 0x50, 0x4a,
        0x98, 0xba, 0xfa, 0x28, 0x80, 0x9d, 0x61, 0x12,
    ]);

    /// VMware Reserved.
    pub const VMWARE_RESERVED: Self = Self([
        0x8d, 0x61, 0x00, 0x99, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ]);
}
...
impl Debug for GptPartitionName {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        write!(f, "GptPartitionName({:?})", core::str::from_utf8(&s[..len]).unwrap_or("?"))
    }
}
