...
pub async fn plan<C: Clock + Clone>(tree: &Tree, opts: &CdOptions<C>) -> Result<Report, PathError> {
...
/// either writer keep their kind, detail code and path.
pub async fn write<D: BlockDevice, C: Clock + Clone>(mut out: D, tree: &Tree, opts: &CdOptions<C>) -> Result<Report, PathError> {
    let udf_opts = bridge(opts);
    let udf = super::udf::plan(tree, &udf_opts).await?;
    let iso_opts = opts.iso().clone().with_min_blocks(opts.iso().min_blocks().max(udf.allocated_end()));
...
async fn cover<D: BlockDevice, C: Clock>(out: &mut D, opts: &IsoOptions<C>, blocks: u64) -> Result<(), hadris_fs::Error<D::Error>> {
