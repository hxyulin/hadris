    ...
    S: Read + Seek<Error = <S as Read>::Error>,
{
    ...
    Udf(hadris_udf::r#async::fs::UdfFs<Borrowed<'a, S>>),
}
    ...
    S: Read + Seek<Error = <S as Read>::Error>,
{
    ...
    pub async fn open(source: &'a mut S, policy: OpenPolicy) -> Result<Self> {
        let formats = crate::detect::r#async::detect(source)
            .await?
            .ok_or(Error::UnknownFormat)?;
        Self::open_detected(source, formats, policy).await
    }
    ...
    pub async fn open_detected(
        ...
        match selected {
            ...
            OpticalFormat::Udf => hadris_udf::r#async::fs::UdfFs::open(Borrowed::new(source))
                .await
                .map(Self::Udf)
                .map_err(Error::Udf),
        }
    ...
    pub fn as_udf(&self) -> Option<&hadris_udf::r#async::fs::UdfFs<Borrowed<'a, S>>> {
        match self {
    ...
    pub fn as_udf_mut(&mut self) -> Option<&mut hadris_udf::r#async::fs::UdfFs<Borrowed<'a, S>>> {
        match self {
            Self::Udf(image) => Some(image),
            Self::Iso9660(_) => None,
        }
