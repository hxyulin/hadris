...
These images are generated from the fixtures in this directory. Run commands
from the repository root.

Regenerate them with the current release after any change to emitted bytes:
the 2.0.0 ISO, UDF, and bridge conformance fixes altered output for images
that previously deviated from the relevant standard, so images generated by
earlier release candidates are not byte-comparable with current output.
