...
| FAT:LFN | `RawLfnEntry` | partial | `lfn::tests::checksum_matches_reference`, `lfn::tests::encoded_orders_entries_last_first`, `lfn::tests::assembler_rejects_broken_sequences`, `fatfs_write::long_names_up_to_255_units`, `raw::dirent::tests::layouts_match_the_specification` | `fat_read` | Sequence, attributes, checksum, terminator and filler are read and written; names are UTF-16 only, with no legacy ANSI fallback. |
| FAT:DirEntry | `RawDirEntry` | partial | `dirent::tests::decodes_short_fields`, `fatfs_write::short_names_and_case_bits`, `raw::dirent::tests::layouts_match_the_specification` | `fat_read` | Name/attributes/timestamps/cluster/size and NT case flags (`DIR_NTRes`) are read and written; extended access-time granularity is not modeled. |
