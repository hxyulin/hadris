...
|---|---|---|---|---|---|---|---|---|---|
| IO-OPEN-01 | Open a filesystem on a block device whose logical block size is 512, 1024, 2048 or 4096 bytes, and reject larger sizes with Unsupported, not corruption | core | Y 512-4096 | Y 512-4096 | Y 2048 normal; 512-2048 legal | Y 512-4096 | - byte stream | all | 3.0: 4Kn disks and optical media are common |
...
| VOL-LABEL-01 | Read the volume label as text. FAT: the root label entry, falling back to the BPB. exFAT: UTF-16 label entry. ISO: PVD volume id, or the Joliet SVD id when it is richer. UDF: logical volume id | core | Y | Y | Y | Y | N | all | 3.0: decision D2 (`label()` everywhere, defaulted `volume_label()` on the trait) |
...
| VOL-CAPS-01 | Query capabilities before acting: writable, symlinks, hard links, case sensitivity, name length and charset, time resolution per field | core | Y | Y | Y | Y | Y | ker, host | 3.0, including which metadata fields and timestamps are stored (needed by the META-TIME-02 rule); per-field time resolution 3.x (4.14) |
...
| ID | Action (edge case that matters) | Class | F | X | I | U | C | Users | Want |
|---|---|---|---|---|---|---|---|---|---|
| DIR-LOOKUP-01 | Look up a name in a directory using the format's rules: FAT and exFAT case-insensitive and case-preserving (exFAT through the volume's up-case table and name hash); ISO primary matches without `;1` and without case; RR, Joliet, UDF and cpio match exactly | core | Y | Y | Y | Y | Y (path in archive) | all | 3.0; cpio 3.x (random access needs the 3.x read-only driver, decision 4) |
...
| DIR-LOOKUP-03 | Look up an ISO name with several versions (`A.TXT;1`, `A.TXT;2`) and get the highest; look up `README` against the stored `README.;1` | core | - | - | Y | - | - | insp, ker | 3.0: ECMA-119 7.5 |
...
| DIR-RMDIR-02 | Remove a tree recursively (remove_dir_all) without an allocator, with a bounded depth that gives LimitExceeded | core | Y | Y | - | Y (rw) | - | emb, host | 3.0 |
...
| FILE-UNLINK-01 | Remove a file and free its clusters; remove on a directory with RemoveKind::File gives IsADirectory; exFAT also frees clusters owned by vendor allocation entries | core | Y | Y | B | Y (rw) | - | all | 3.0 |
...
| FILE-CLOSE-01 | Close a handle with a Result, publishing size and times; dropping a written handle publishes its size best effort, so a power cut keeps the data | core | Y | Y | Y | Y | - | emb, all | 3.0: embedded-8 (2205 of 2250 bytes lost) |
...
| ID | Action (edge case that matters) | Class | F | X | I | U | C | Users | Want |
|---|---|---|---|---|---|---|---|---|---|
| META-STAT-01 | stat: type, size, times, link count, and permissions and owner where the format stores them. Directory size follows one documented rule for every driver (0 on FAT, extent size on ISO today) | core | Y | Y | Y | Y | Y | all | 3.0: directory size is 0 on every driver, documented (decided 2026-09-24) |
...
| META-TIME-02 | Set each timestamp separately (utimensat). A field the format cannot store fails with Unsupported unless the value equals what the format already reports; VOL-CAPS-01 says which fields are stored | core | Y | Y | B | Y (rw) B | B | all | 3.0: one rule for every unstorable field (decided 2026-09-24) |
...
| META-OWNER-02 | chown on a format without owners fails with Unsupported unless the value equals the reported owner (rule of META-TIME-02); host copy helpers strip unstorable fields using VOL-CAPS-01 | core | N | N | B | Y (rw) B | B | ker | 3.0: policy only |
...
| META-INO-01 | A stable node identity: NodeId unchanged by rename; every name of a hard link has the same id (RR via PX serial or data extent, UDF via the ICB, cpio via dev and ino) | core | Y | Y | Y | Y | Y | ker, insp | 3.0: A7 |
...
| HOST-ERRNO-01 | Map every error kind to an errno (EPERM vs ENOSYS for Unsupported, EROFS, ENOTEMPTY, ELOOP, ENAMETOOLONG) | core | Y | Y | Y | Y | Y | ker | 3.0: integrate-17, promised in docs |
...
| NF-STABLE-02 | UDF read-write mount (VOL-MOUNT-05, VOL-FORMAT-06, LINK-HARD-02, FILE-UNLINK-02 on UDF) can be added in 3.x purely additively: the 3.0 UDF type and the mounted-filesystem API need no signature change. The prototype must show it | 3.0 |
